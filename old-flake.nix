{
  ...
  inputs = {
    # Nixpkgs
    nixpkgs.url = "github:nixos/nixpkgs/nixos-unstable"; # done
    masterNixpkgs.url = "github:NixOS/nixpkgs/master"; # done

    home-manager.url = "github:nix-community/home-manager/master"; # done
    home-manager.inputs.nixpkgs.follows = "nixpkgs"; # done

    darwin.url = "github:lnl7/nix-darwin/master"; # done
    darwin.inputs.nixpkgs.follows = "nixpkgs"; # done

    nix-colors.url = "github:misterio77/nix-colors"; # done

    nix-index-database.url = "github:Mic92/nix-index-database"; # done
    nix-index-database.inputs.nixpkgs.follows = "nixpkgs"; # done

    nix-alien.url = "github:thiagokokada/nix-alien"; # done
    nix-alien.inputs.nixpkgs.follows = "nixpkgs"; # done

    flake-compat.url = "github:edolstra/flake-compat"; # cancel
    flake-compat.flake = false; # cancel

    treefmt-nix.url = "github:numtide/treefmt-nix"; # done
    treefmt-nix.inputs.nixpkgs.follows = "nixpkgs"; # done
  };
