...
let
  ...
  qos-prestart = pkgs.writeScriptBin "qos-prestart" ''
    #!${pkgs.stdenv.shell}
    ...
    # Check if interactive_ports exists before using it
    if ${ipset} list interactive_ports >/dev/null 2>&1; then
    ...
    # Check if nix_hosts exists before using it
    if ${ipset} list nix_hosts >/dev/null 2>&1; then
  ...
  qos-script = pkgs.writeScriptBin "qos-script" ''
...
in
{
  config = lib.mkIf cfg.enable {
    systemd.services.network-qos = {
      ...
      preStart = "${qos-prestart}/bin/qos-prestart";
      script = "${qos-script}/bin/qos-script";
